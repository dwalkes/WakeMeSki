...
public class AlarmCalculator {
	...
	/**
	 * @return a calendar object representing the next time the alarm should fire, or 
	 * null if no day is selected for the next alarm
	 */
	public Calendar getNextAlarm() {
		...
		// never set less than 1 minutes ahead - always go to the next day in this case
		nextAlarm.add(Calendar.MINUTE, 1);
		if( nextAlarm.get(Calendar.HOUR_OF_DAY) <= setHour  &&
			nextAlarm.get(Calendar.MINUTE) < setMinute &&
			mDaySelect.isDaySelected(nextAlarm.get(Calendar.DAY_OF_WEEK)) ) {
			...
			for( ; i<7; i++ ) {
				...
				if( mDaySelect.isDaySelected(nextAlarm.get(Calendar.DAY_OF_WEEK))) {
					break;
				}
			}
			if( i==7 ) {
