...
import com.dwalkes.android.ListPreferenceMultiSelect;

import android.util.Log;

/**
 * A POJO used to access repeat day information shared in an android
 * preference.  This POJO is shared by the preference activity and the
 * code reading directly from the shared preference datastore.
 * 
 * @author dan
 *
 */
public class RepeatDaySharedPreference {
	 ...
	 * @param persistentString  The persistent string read from the database.
	...
	public boolean setFromPersistString( String persistentString ) {
		if( persistentString != null ) {
			selectedDays = ListPreferenceMultiSelect.parseStoredValue(persistentString);
		} else {
			selectedDays = null;
		}
