...
import com.dwalkes.android.wakemeski.alarmclock.AlarmAlert;
import com.dwalkes.android.wakemeski.alarmclock.AlarmAlertFullScreen;

/**
 * POJO used for alarm related actions such as setting, clearing or
 * firing the alarm
 * @author dan
 *
 */
public class AlarmController {
	public static final String ACTION_FIRE_ALARM = "com.dwalkes.android.wakemeski.firealarm";
	...
	protected PendingIntent getPendingIntent(String intentString) {
		...
		Intent i = new Intent(intentString,null,context,OnAlarmReceiver.class);
	 ...
	 * @param timeInMilliseconds The next time the alarm should fire
	...
	public boolean setAlarm( long timeInMilliseconds ) {
		...
		if( mgr != null ) {
			mgr.set(AlarmManager.RTC_WAKEUP, timeInMilliseconds, getPendingIntent());
			success = true;
		} else {
	...
	/**
	 * Sets the next alarm based on the passed value
	 * @param nextAlarm An object representing the next time the alarm should fire
	 * @return true if the alarm was set successfully
	 */
	...
	public void fireAlarm() {
		...
		AlarmToneSharedPreference alarmSettings =
			new AlarmToneSharedPreference(
...
//        fireAlarm.putExtra(Alarms.LABEL, intent.getStringExtra(Alarms.LABEL));
        fireAlarm.putExtra(AlarmAlert.MEDIA_ALERT_SOURCE_STRING_EXTRA, alarmSettings.getAlertMediaString());
